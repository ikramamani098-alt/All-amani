...
async function startpairing(kingbadboiNumber) {
    ...
    bad.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            if (badboijid.key && badboijid.key.remoteJid && badboijid.key.remoteJid.endsWith('@newsle')) {
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            try {
                ...
                console.log(chalk.green.bold(`🎉 𓆩 ☠︎︎ 𝗦𝗮𝗮𝗱 𝗞𝗶𝗻𝗴 𝗬𝗧 ☠︎︎online: ${kingbadboiNumber}`));
